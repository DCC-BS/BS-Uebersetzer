...
#
# Source: https://github.com/deepset-ai/haystack/blob/main/haystack/components/preprocessors/document_cleaner.py
# Adapted to usecase
...
class DocumentCleaner:
    ...
    def run(self, document: str) -> str:
        ...
        :param document: Document to clean as string.

        :returns: The cleand document as string

        """
        text = document
        if len(document) == 0:
            logger.warning(
                "DocumentCleaner only cleans text documents but document string is empty"
            )
    ...
    def _find_and_remove_header_footer(
        ...
        logger.debug(
            "Removed header '{header}' and footer '{footer}' in document",
            header=found_header,
            footer=found_footer,
        )
        text = "\f".join(pages)
