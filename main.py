...
def main(input_file: str):
    if input_file.endswith(".docx"):
        doc, original_text = load_content(input_file)
        translated_doc = translate_text(
            original_text, target_language="German", doc=doc
        )

        # Save the translated document
        output_file = input_file.replace("input", "output/nmt")
        ...
        llm = "llama3.1:70b"
        translated_text_llm = chunk_and_translate(
            original_text,
        ...
        generate_comparison_output(
            original_text, translated_text_llm, output_file_llm, translation_method=llm
        )
        generate_comparison_output(
            original_text,
            translated_text_nmt,
            output_file_nmt,
            translation_method="Opus NMT",
        )
